...
export default {
  ...
  methods: {
    //鼠标进入的方法
    enterHandler: throttle(function (index, e) {
      //修改响应式数据
      this.currentIndex = index;
      e.target.parentNode.parentNode.parentNode.style.width = "946px";
      //鼠标进入事件,假如用户的行为过快,会导致项目业务丢失【里面业务有很多，可能出现卡顿现象】。
      //一句话：用户行为过快,浏览器反应不过来,导致业务丢失!!!!
      //函数的防抖与节流技术
      // console.log(e.target.parentNode.parentNode.parentNode.style.width);
    }, 10),
    //鼠标移出事件
    leaveHandler(e) {
      //鼠标移出高亮的效果消失
      e.target.lastChild.style.width = "210px";
  ...
  computed: {
    ...
    ...mapState({
      ...
      // categoryList: (state) => {
      //   return Istate.home.categoryList;
      // },
    }),
    // ...mapState('home',{category:'category'}),
